...
for i in range(3):
    ...
    #获取市的名称
    province_name = input("\033[34;1m请输如您要查看的省名:\033[0m ")
    #定义标志位用来跳出整个循环
    jump_up_flag = False
    #检查输入是否为正确的省名
    if province_name in city_map:
        ...
        while True:
            ...
            print("----------------------------------------------------")
            shi_pl = input("\033[33;1m请输入你要查看的市名：\033[0m")  #获取用户的输入
            if shi_pl in shi_nl: #如果市名存在
                print("\033[33;1m-----------------%s包含的县为--------------------\033[0m" % shi_pl)
                xian_nl = city_map[province_name][shi_pl]  #获取县列表
                for xian_list in xian_nl: #循环打印县列表
                ...
                continue
            jump_shi = input("\033[32;1m请问是否退出：1 退出、2 返回最上层,任意输入返回上一层：\033[0m")
            '''判断用户的输入：如果输入1则返回上级目录并且修改标志位的值为True，在上一层代码有判断标志位的代码
            如果标志位为True执行break将跳出整个循环
            如果输入2.直接break跳出while循环到最上级目录！
            '''
            if jump_shi == "1":   #获取用户输入，并修改标志位为True
                jump_up_flag = True
                break
            if jump_shi == "2":
                break
    if jump_up_flag: #判断如果标志位为True，这里将执行，然后执行下面break代码退出整个循环。
        break
    else:
