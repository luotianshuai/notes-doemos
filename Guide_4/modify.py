...
def add_backend(backend):
    ...
    if check_backend:
        ...
        with open('(.conf', 'r') as old_ha,open('haproxy.conf.new', 'w') as new_ha:
            for line in old_ha:
                new_ha.write(line)
            new_ha.write('\n')
...
def del_backend(backend):
    backend_title = backend.get('backend')
    current_title = 'backend %s' % backend_title
    current_record = 'services %s %s weight %s maxconn %s' % \
                     (backend['record']['services'], backend['record']['services'], backend['record']['weight'],
                      backend['record']['maxconn'])
    check_backend = get_backend(backend_title)
    if check_backend:
        '''如果backend存在'''
...
输入3删除ha记录
输入0将退出程序\033[0m''')
        num = input('\033[33;1m请输入您需要的功能：\033[0m')
        if num == '1':
            print('''查询功能测试：
存在：www.oldboy.org buy.oldboy.org nb.oldboy.org  不存在：none.oldboy.org''')
            read = input('\033[33;1m请输入您要查看的backend名称：\033[0m')
        ...
        if num == '2':
            print('\033[32;1m输入添加测试：backend不存在：\033[33;1m{"backend": "test.oldboy.org","record":'
                  '{"services": "100.1.7.9","weight": 20,"maxconn": 3000}}\033[0m\033[0m')
            print('\033[32;1m输入添加测试：backend存在record存在：\033[33;1m{"backend": '
                  '"buy.oldboy.org","record":{"services": "100.1.7.10","weight": 20,"maxconn": 3000}}\033[0m\033[0m')
            print('\033[32;1m输入添加测试：backend存在record不存在：\033[33;1m{"backend": "buy.oldboy.org","record":'
                  '{"services": "100.1.7.101","weight": 20,"maxconn": 3000}}\033[0m\033[0m')
            read = input('\033[33;1m请输入您要添加的信息：\033[0m')
            read_new = json.loads(read)
            print(add_backend(read_new))
        if num == '3':
            print('\033[32;1m输入删除测试：backend不存在：\033[33;1m{"backend": "test.oldboy.org","record":{"services":'
                  '"100.1.7.9","weight": 20,"maxconn": 3000}}\033[0m\033[0m')
            print('\033[32;1m输入删除测试：backend存在record存在,且仅有一条record记录：\033[33;1m{"backend":'
                  '"www.oldboy.org","record":{"services": "100.1.7.9","weight": 20,"maxconn": 3000}}\033[0m\033[0m')
            print('\033[32;1m输入删除测试：backend存在record存在,且有其他record记录：：\033[33;1m{"backend":'
                  '"buy.oldboy.org","record":{"services": "100.1.7.10","weight": 20,"maxconn": 3000}}\033[0m\033[0m')
            print('\033[32;1m输入删除测试：backend存在record不存在：\033[33;1m{"backend": '
                  '"buy.oldboy.org","record":{"services": "100.1.7.99","weight": 20,"maxconn": 3000}}\033[0m\033[0m')
            read = input('\033[33;1m请输入您要删除的信息：\033[0m')
            read_new = json.loads(read)
